...
2. [x] Setup server endpoint to handle POST requests  
   **Validation:** POST requests received at `/api/frame`
3. [x] Add payload decoding/encoding utilities  
   **Validation:** Round-trip encode/decode test passes
4. [x] Create state schema for previous followers/unfollows  
...
11. [x] Convert timestamps to UNIX format  
...
### API
1. [x] Create API client for Farcaster followers endpoint  
...
3. [x] Implement API retry logic with exponential backoff  
    **Validation:** Recovers from 3 consecutive failures

### UI
1. [x] Implement basic refresh button functionality  
    **Validation:** Click triggers frame reload
2. [x] Create template helpers for rendering user profiles  
...
4. [x] Add empty state handling  
    **Validation:** Shows "No unfollows" message
5. [x] Create error state templates  
    **Validation:** Displays error message + retry button
6. [x] Add loading indicators during API calls  
